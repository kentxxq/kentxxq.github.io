...
- ai 调用 playwright 的话，可以用这个 https://github.com/lightpanda-io/browser
- https://crawlee.dev/ 使用 playwright
- [scrapy](https://github.com/scrapy/scrapy)   python 的 http
...
- 类似 obsidian 和 codex 的缩进布局
	- https://github.com/antoniandre/splitpanes
	- https://github.com/bvaughn/react-resizable-panels
	- 或者自己写，就是 grid 三列布局，或者 flex？ 然后点击隐藏即可。
...
- [HybridCache](https://learn.microsoft.com/en-us/aspnet/core/performance/caching/hybrid) 官方支持
	- HybridCache 用法是把实际获取数据的方法包一层。
	- 有 2 层缓存，防击穿。实现简单
	- 缺点：所以无法和 cache 特性一起使用。
- [fusionCache](https://github.com/ZiggyCreatures/FusionCache) 比较流行，高复杂度场景。兼容官方
- [easycaching]( https://github.com/dotnetcore/easycaching ) 国产，可以用特性。但是不兼容 rougamo aop。维护不是特别积极了
	- 如果要 cache 特性来做，那么就是 rougamo+IDistributedCache 读取缓存。命中返回。否则执行原方法，成功后写入缓存。缺点：没有了 HybridCache 的缓存击穿，2 级缓存
	- 或者不用 rougamo，用 aspectcore 或者 castle。他们是运行时代理，所以天然支持
...
> 并发和队列？
> 通过 mq 来做队列，然后消费。
> 并发通过 backgroundservice 跑一个循环任务。然后用 Parallel.ForEachAsync 控制并发大小即可。

这里只做定时任务，以及处理重试即可。

- quartz 定时任务。定时任务比较强，也支持临时触发
- hangfire ，pro 要收费。后台任务
- tickerQ 源生成，比较新，也比较强
- [GitHub - jamesmh/coravel: Near-zero config .NET library that makes advanced application features like Task Scheduling, Caching, Queuing, Event Broadcasting, and more a breeze!](https://github.com/jamesmh/coravel) 似乎维护不积极了
...
- [全球光污染地图 | 实时查询光害&中国暗夜环境等级 - 天文通](https://www.darkmap.cn/)
...
- [https://phyphox.org/](https://phyphox.org/) 查看手机上的各种传感器数据，ios/安卓

### 终端

- 显卡使用率 [https://github.com/Syllo/nvtop](https://github.com/Syllo/nvtop)
...
- 搜片
	- [laoma2053/awesome-zhuiju-free: 免费无广告的追剧资源指南，人工精选资源、每天检测资源有效性。收录在线影视、影视APP、网盘搜索、磁力BT、字幕、TVBox / 影视仓空壳软件/配置地址、IPTV直播源、会员拼团、影视相关开源项目。开源，社区共同维护。](https://github.com/laoma2053/awesome-zhuiju-free)
	- [fish2018/pansou: PanSou是一款高性能的网盘资源搜索API服务，支持TG频道和插件搜索。系统设计以性能和可扩展性为核心，支持多频道多插件并发搜索、结果智能排序和网盘类型分类。docker集成前后端，一键启动，开箱即用。仅供学习研究，请勿以各种形式用于盈利目的。](https://github.com/fish2018/pansou)
- iptv
...
- https://github.com/leaperone/smctl 调用 mac 没有开放出来的硬件 api 功能
- 视频播放
...
- [LanRhyme/MicYou: 安卓设备做其他设备的麦克风](https://github.com/LanRhyme/MicYou)
...
- 看图。要比 imageglass 快的更新慢或者维护不好，暂时用 imageglass
	- [ImageGlass](https://github.com/d2phap/ImageGlass)
	- [Honeyview · Free Image Viewer · Download & Features](https://fr.bandisoft.com/honeyview/)
	- [IrfanView - Official Homepage - One of the Most Popular Viewers Worldwide](https://www.irfanview.com/)
	- [diffractor/diffractor: The fastest photo and video organizer for Windows.](https://github.com/diffractor/diffractor)
	- [XnView MP · Download | Free | XnView.com](https://www.xnview.com/en/xnview-mp/)
	- [neelabo/NeeView: An image viewer that allows you to browse images in folders and compressed files like a book. Powerful customization is available.](https://github.com/neelabo/NeeView)
	...
	- [nomacs/nomacs: nomacs is a free image viewer for windows, linux, and mac systems.](https://github.com/nomacs/nomacs)
...
- [winsiderss/systeminformer: 无需安装的 windows 面板监控](https://github.com/winsiderss/systeminformer)
...
- [KillerPDF 极小的 windows pdf 浏览，编辑工具](https://github.com/SteveTheKiller/KillerPDF)
- [iOfficeAI/OfficeCLI: 命令行操作 office，给 ai 很好用](https://github.com/iOfficeAI/OfficeCLI)
	- 在线 office 预览，doc 转 html canvas [yukiyokotani/office-open-xml-viewer: A browser-based viewer for Office Open XML documents that renders to an HTML Canvas element.](https://github.com/yukiyokotani/office-open-xml-viewer)
...
- 截图 + 录屏 +gif
    ...
    - macos 开源截屏工具，看上去不错 [realskyrin/capcap: A lightweight, native macOS screenshot tool that lives in your menu bar. Double-tap ⌘ Command to capture any region of your screen — instantly copied to clipboard, or annotate first with pen and mosaic tools.](https://github.com/realskyrin/capcap)
...
- 编码操作 web 页面
    - [playwright](https://github.com/microsoft/playwright) 比较新，微软支持，支持的语言多。puppeteer 作者在微软新写的
...
- [velopack/velopack: 跨平台，把你的应用打包，支持增量更新](https://github.com/velopack/velopack)
...
- 股票 api [API 概览 | Stock SDK](https://chengzuopeng.github.io/stock-sdk/api/)
...
- `k3s`, `k0s` [k0sproject/k0s: k0s - The Zero Friction Kubernetes](https://github.com/k0sproject/k0s)
