...
# 查看 uuid，
blkid
# uuid 更推荐，不会因为换机器，调整磁盘位置而改变
uuid=123456 /data xfs defaults  0  0
# 理论上路径仍然依赖 device-mapper，
/dev/vg_data/lv_data /data xfs defaults  0  0
```

> 如果是阿里云, 参考 [[阿里云手册#新加硬盘]]
...
history -c
# 删除最近 10 条
for i in {1..10}; do history -d $(history 1 | awk '{print $1}'); done && history -w
