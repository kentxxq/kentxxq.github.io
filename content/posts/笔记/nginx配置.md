...
server {
    http2 on;
    listen 443 ssl;
    server_name demo.kentxxq.com esa-demo-cdn.kentxxq.com;
    include /usr/local/nginx/conf/options/ssl_kentxxq.conf;
    #ssl_certificate /usr/local/nginx/conf/ssl/kentxxq.cer;
    #ssl_certificate_key /usr/local/nginx/conf/ssl/kentxxq.key;

    access_log /usr/local/nginx/conf/hosts/logs/demo.kentxxq.com.log;
    include /usr/local/nginx/conf/options/allow_all_cross_origin.conf;

    location /demo-files/ {
        include /usr/local/nginx/conf/options/allow_all_options_cross_origin.conf;
        alias /data/demo-files/;
    }

    root /data/html/demo;
    index index.html;

    # 明确返回 404 的请求展示 Nuxt 404 页面。
    error_page 404 /404.html;

    # 直接请求 HTML 时禁用缓存；其他已存在的静态资源不添加 no-store。
    location / {
    ...
    # /app -> /app/index.html；未生成的前端路由回退到 /index.html。
    # 此处返回的内容均为 HTML，因此统一禁用缓存。
    location @index {
        add_header Cache-Control "no-store" always;
        try_files $uri/index.html /index.html;
    }
