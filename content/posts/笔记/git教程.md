...
github 的 22 端口被阻断，`vim ~/.ssh/config`
...
- [码农高天视频教学](https://www.youtube.com/watch?v=uj8hjLyEBmU)
    1. `git rebase main` 可以让你的改动是基于最新的代码, 如果操作这一步有冲突, 就解决冲突
	    1. vscode 内解决冲突以后，`git add .`
	    2. `git rebase --continue` 会进入到一个 vim 类似的界面，只有非注释内容会变成 commit message
	    3. 这时候看 git 记录，会发现有同步和上传，这时候你应该强制 push 到自己的开发分支
