...
vars:
  ...
  REMOTE_HOST: '{{.REMOTE_HOST | default "q.kentxxq.com"}}'
  REMOTE_TARGET: '{{.REMOTE_TARGET | default "/tmp"}}'

  # 默认不开启删除功能
  DELETE_ON_SYNC: '{{.DELETE_ON_SYNC | default "false"}}'

tasks:
  # 一键同步
  sync:
    desc: '执行同步任务 (DELETE_ON_SYNC: {{.DELETE_ON_SYNC}})'
    vars:
      _SOURCE_FLAGS: '{{range .SOURCES}}"{{.}}" {{end}}'
      _EXCLUDE_FLAGS: '{{range .EXCLUDE_LIST}} --exclude="{{.}}" {{end}}'
      _DELETE_FLAG: '{{if .DELETE_ON_SYNC}}--delete{{end}}'
    cmds:
      - 'echo ">>> [SSH-DEPLOY] 同步启动 (清除模式: {{.DELETE_ON_SYNC}}) -> {{.REMOTE_HOST}}:{{.REMOTE_TARGET}}"'
      - 'rsync -avz {{._DELETE_FLAG}} {{._EXCLUDE_FLAGS}} {{._SOURCE_FLAGS}} {{.REMOTE_USER}}@{{.REMOTE_HOST}}:{{.REMOTE_TARGET}}'

  # 远程执行
  exec:
    desc: '在远程终端运行命令: REMOTE_CMD (用法: task base:exec -- REMOTE_CMD="ls -l")'
    cmds:
      - 'ssh {{.REMOTE_USER | default "root"}}@{{.REMOTE_HOST}} "cd {{.REMOTE_TARGET | default "/tmp"}} 2>/dev/null || true; {{.REMOTE_CMD}}"'

  # 连接检查
  check:
    desc: "测试连接稳定性"
    cmds:
      - 'ssh -o ConnectTimeout=5 {{.REMOTE_USER | default "root"}}@{{.REMOTE_HOST}} "echo 成功连接到 {{.REMOTE_HOST}}"'
```
